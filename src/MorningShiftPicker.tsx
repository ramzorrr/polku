...
/**
 * For a 'morning shift' scenario:
 * Sign in time => 05:xx or 06:xx  (05:45..06:15)
 * Sign out time => 13:xx or 14:xx (13:45..14:15)
 */
function MorningShiftPicker({ onChangeStartTime, onChangeEndTime }: MorningShiftPickerProps) {
  // Default start time: hour=5, minute=45 (05:45)
  const [startHour, setStartHour] = useState<number>(5);
  const [startMinute, setStartMinute] = useState<number>(45);

  // Default end time: hour=13, minute=45 (13:45)
  ...
  /**
   * START (Sign in) logic
   * Valid hours for sign in => [5, 6]
   * If hour=5 => minutes must be >=45..59
   * If hour=6 => minutes must be <=15..0
   */
  const handleStartHourChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // if hour=5 => must be minute >=45
    if (newHour === 5 && newMin < 45) {
      newMin = 45;
    }
    // if hour=6 => must be minute <=15
   ...
   * END (Sign out) logic
   * Valid hours => [13, 14]
   * If hour=13 => must be minute >=45..59
   * If hour=14 => must be minute <=15..0
   */
  ...
  return (
    <div className="p-2 border rounded">
      <p className="font-bold">Aamuvuoro</p>
